...
export const UpdateEmployeeSchema = CreateEmployeeSchema.partial().omit({ username: true, password: true }).extend({
...
export const IdParmaSchema = z.object({
    id: z.string().transform((v) => parseInt(v, 10)).refine((v) => !isNaN(v), { message: "Invalid ID" })
});
